...
void View::DrawMessages()
{
    ...
    static const char* const sourceNames[(size_t)MessageSourceType::COUNT] = { ICON_FA_USER, ICON_FA_MICROSCOPE };
    ...
    static const char* const severityNames[(size_t)MessageSeverity::COUNT] = { "Trace", "Debug", "Info", "Warning", "Error", "Fatal" };
    static const char* const severityIcons[(size_t)MessageSeverity::COUNT] = { ICON_FA_SHOE_PRINTS, ICON_FA_BUG, ICON_FA_INFO, ICON_FA_TRIANGLE_EXCLAMATION, ICON_FA_CIRCLE_XMARK, ICON_FA_SKULL_CROSSBONES};

    for( int i=0; i<(int)MessageSeverity::COUNT; i++ )
    {
        if( i != 0 ) ImGui::SameLine( 0.0, buttonSpacing );
#if 0 // Set to 1 to have the message count displayed out of the button
        char buffer[128];
        if( m_visibleMessagesPerSeverity[i] == m_messagesPerSeverity[i] )
        {
            snprintf( buffer, sizeof( buffer ), "%s", RealToString( m_messagesPerSeverity[i] ) );
        }
        else
        {
            snprintf( buffer, sizeof( buffer ), "%s / %s", RealToString( m_visibleMessagesPerSeverity[i] ), RealToString( m_messagesPerSeverity[i] ) );
        }
        FilterButton( severityIcons[i], ImVec2( 0, 0 ), m_messageFilter.m_showMessageSeverityFilter[i], buffer );
#else
        char buffer[128];
        if (m_visibleMessagesPerSeverity[i] == m_messagesPerSeverity[i])
        {
