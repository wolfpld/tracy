...
Enabling the \emph{Limit frame range} option allows you to manually set the frame image playback region. Adjustments can be made using the start and end sliders or by copying the time range from either an annotation (section~\ref{annotationlist}) or one of the time range limits (section~\ref{timerangelimits}).
...
\item \emph{\faArrowsLeftRightToLine{}~Limit to view} -- Set the time range limit to current view.
\item \emph{\faNoteSticky{}~Add annotation} -- Create a new annotation matching the time range.
