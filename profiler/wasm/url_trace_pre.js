// URL-mode trace loader: opens ?t=<url> as the startup trace instead of embed.tracy.
if( typeof document !== 'undefined' )
{
    const tracyTraceUrl = new URLSearchParams( window.location.search ).get( 't' );
    if( tracyTraceUrl )
    {
        let tracyUrl = null;
        let tracyUrlError = null;
        try { tracyUrl = new URL( tracyTraceUrl, window.location.href ); }
        ...
        // Emscripten builds compile without exception catching, so FileRead's
        // header validation aborts the page instead of unwinding; gate in JS.
        ...
        Module.preRun.push( () =>
        {
            addRunDependency( 'tracy-url-trace' );
            tracyTraceData.then(
                ( bytes ) =>
                {
                    if( !isTracyFile( bytes ) )
                    {
                        ...
                        alert( 'Cannot load trace from ' + tracyTraceUrl + ': not a Tracy trace file' );
                    }
                ...
                {
                    FS.writeFile( '/url.tracy.failed', new Uint8Array( 0 ) );
                    alert( 'Cannot load trace from ' + tracyTraceUrl + ': ' + err );
                } ).finally( () => removeRunDependency( 'tracy-url-trace' ) );
        } );
    }
}
