...
Tracy provides an [MCP (Model Context Protocol)](https://modelcontextprotocol.io) server that allows AI assistants to perform deep performance analysis directly on Tracy captures. It runs as a Python sidecar process, using C++ server bindings to access Tracy's analysis engine.
...
The server runs as a **singleton** on SSE transport (port 47380 by default). Only one
process loads `TracyServerBindings` regardless of how many editor windows are open;
subsequent launches detect the port is taken and exit immediately.

Start the server once before opening your editor:
...
Configure your AI assistant using that URL:
...
The server prefers port 47380 (override with `TRACY_MCP_PORT`). If that port is
taken by another application, it scans upward automatically and falls back to any
OS-assigned free port. Check `tracy_mcp.port` or the startup output for the
actual port if it differs from the default.

Set the environment variables before launching the server (or export them in your shell):

    PYTHONPATH=/path/to/tracy/build/python/Release
    TRACY_CAPTURES_DIR=/path/to/captures
    TRACY_MCP_PORT=47380        # optional; default 47380

Setting `TRACY_CAPTURES_DIR` enables the `list_captures` tool, which scans that directory (top-level only) for `*.tracy` files so the assistant can discover and load traces by name without needing full paths.
...
Inside `eval`, the variable `ctx` is a `Worker` instance. The following methods are available:

**Capture metadata**

- `get_capture_name()` / `get_capture_program()` --- Name and program string stored in the trace.
- `get_capture_time()` --- Unix timestamp of the capture.
- `get_host_info()` --- OS, CPU, RAM, and compiler info as a string.
- `get_pid()` --- Process ID of the profiled application.
- `get_resolution()` --- Timer resolution in nanoseconds.
- `get_first_time()` / `get_last_time()` --- Trace time range in nanoseconds.
- `get_cpu_manufacturer()` --- CPU manufacturer string.

**Counts**

- `get_zone_count()` / `get_gpu_zone_count()` --- Total CPU and GPU zone events.
- `get_lock_count()` / `get_plot_count()` / `get_context_switch_count()` --- Counts for other data types.
- `get_src_loc_count()` / `get_callstack_sample_count()` / `get_message_count()`

**CPU zones**

- `get_all_zone_stats()` --- Returns a `dict[str, ZoneStats]` keyed by zone name covering all source locations. Each `ZoneStats` has `min`, `max`, `total`, `avg`, `count`, `sum_sq` (all in nanoseconds). Includes nested zones — summing costs will double-count; use `get_root_zone_stats()` for summable per-frame totals.
- `get_root_zone_stats()` --- Like `get_all_zone_stats()` but aggregates only top-level (root) zones per thread. Safe to sum across zones. Use `async_mode=True` on large captures (>50M zones).
- `get_zone_stats(srcloc_id)` --- Stats for a single source location.
- `get_zone_name(srcloc_id)` / `get_src_loc(srcloc_id)`
- `get_zone_source_location(name)` --- Returns `{"name", "function", "file", "line", "color"}` for the named zone. Use this to map hot zones directly to source lines for code navigation.
- `get_zone_durations(name, max_samples=100000)` --- List of individual zone durations (ns) for distribution analysis.
- `get_zone_occurrences(name, max_samples=100000)` --- List of `(start_ns, duration_ns)` pairs for temporal correlation with frames.
- `get_zone_annotations(name, max_samples=10000)` --- List of text strings attached to individual zone invocations via `TracyZoneText`.
- `get_zone_callstacks(name, max_samples=1000)` --- List of resolved callstacks (each a list of `{"name", "file", "line", "addr"}` dicts) for zone invocations that have callstack capture enabled. Use to understand which call paths lead into a hot zone.

**GPU zones**

- `get_all_gpu_zone_stats()` --- Returns a `dict[str, GpuZoneStats]` with the same fields as `ZoneStats`.
- `get_gpu_contexts()` --- Returns a list of `GpuContextSummary` objects with `name`, `type`, `count`, `thread`.
- `get_gpu_zone_durations(name, max_samples=100000)` --- Individual GPU zone durations (ns).
- `get_gpu_zone_occurrences(name, max_samples=100000)` --- List of `(gpu_start_ns, duration_ns)` pairs.

**Frames**

- `get_all_frame_stats()` --- Returns a list of `FrameStats` (one per named frame set) with `name`, `min`, `max`, `avg`, `total`, `count`, `sum_sq`.
- `get_frame_times()` --- Returns a `list[int]` of per-frame durations (nanoseconds) for the default frame set.
- `get_frame_times_named(name)` --- Per-frame durations for a named frame set (e.g. `"GPU"` or `"Frame"`).
- `get_frame_boundaries()` --- Returns a `list[tuple[int, int]]` of `(start_ns, end_ns)` for each frame in the default frame set. Use this to correlate zone activity with specific frames or frame types (e.g. stall vs normal).
- `get_frame_count()` --- Frame count for the default frame set.

**Threads**

- `get_threads()` --- List of `ThreadData` objects with `id`, `count`, `is_fiber`.
- `get_thread_name(tid)` --- Thread name string for a given thread ID.
- `get_thread_context_switches(tid, max_samples=50000)` --- List of `{"start", "end", "cpu", "reason"}` dicts for context-switch events on the given thread.
- `get_cpu_thread_data()` --- Dict of `tid → {"running_time", "running_regions", "migrations"}` aggregated across the trace.
...
- `get_memory_pools()` --- List of `MemPoolSummary` objects with `name`, `high`, `low`, `usage`, `alloc_count`.
- `get_memory_events(max_count=100000, pool_name="")` --- List of raw allocation events: `{"pool", "ptr", "size", "time_alloc", "time_free", "thread_alloc", "callstack_idx"}`. `time_free` is 0 for live allocations. Use with `get_frame_boundaries()` to identify frames with heavy allocation pressure.

**Locks**

- `get_locks()` --- List of `LockSummary` objects with `name`, `is_contended`, `type`, `time_announce`, `time_terminate`, `threads`.
- `get_lock_wait_stats()` --- List of `{"name", "total_wait_ns", "avg_wait_ns", "contention_count", "threads"}` for all contended locks. Only locks with recorded Wait→Obtain pairs are included. Useful for identifying synchronization bottlenecks.

**Callstack sampling**

- `get_callstack_frames(callstack_idx)` --- Resolves a callstack index (e.g. from `get_memory_events`) to a list of `{"name", "file", "line", "addr"}` frame dicts.
- `get_symbol_stats()` --- List of `{"addr", "incl", "excl", "name", "file", "line", "image"}` dicts with inclusive and exclusive callstack-sample hit counts per symbol. `excl` counts frames where this symbol was at the top of the stack (directly executing). Sort by `excl` descending to find hot functions.
- `get_samples_for_symbol(sym_addr)` --- List of `{"time", "thread"}` dicts for all callstack-sample timestamps hitting `sym_addr`. Use to check whether a symbol's cost is concentrated in specific frames or time windows.

**Hardware performance counters** (requires `TRACY_HW_TIMER` and PMU support at capture time)

- `get_hw_sample_summary()` --- List of `{"addr", "name", "file", "line", "image", "samples", "cycles_mean", "retired_mean", "cache_ref_mean", "cache_miss_mean", "branch_ret_mean", "branch_miss_mean", "ipc", "cache_miss_rate", "branch_miss_rate"}` dicts. `ipc` < 1.0 often indicates memory stalls; `cache_miss_rate` > 5% and `branch_miss_rate` > 5% are typical optimization targets. Returns empty list if the capture has no hardware counter data.

**Connection control** (live captures only)

- `is_connected()` / `shutdown()` / `disconnect()`
