...
\item \texttt{TracyCLockBeforeUnlock(lock\_ctx)}
...
Before you release the lock use the \texttt{TracyCLockBeforeUnlock} macro:

\begin{lstlisting}
TracyCLockBeforeUnlock(tracy_lock_ctx);
ReleaseMutex(lock);
\end{lstlisting}
...
\item \texttt{TracyCSharedLockBeforeUnlock(lock\_ctx)}
...
\item \texttt{TracyCSharedLockBeforeSharedUnlock(lock\_ctx)}
...
Deadlock detection (section~\ref{deadlockdetection}) shows a red \faArrowsSpin{}~icon whenever it finds a cycle of threads waiting on one another's locks. Hovering over the icon lists the deadlocked threads and the locks involved.
...
\item \emph{\faThumbtack{}~Pin} -- Pins the track so that it remains visible while scrolling the timeline view vertically. Pinned threads and GPU queues are anchored to the top of the view, while pinned plots are anchored to the bottom. Selecting \emph{Unpin} on a pinned track releases it.
\end{itemize}
...
Tracy analyzes the recorded lock events for deadlocks. It builds a wait-for graph from the collected events. When the graph forms a closed cycle, all threads in it are reported as deadlocked. A single thread upgrading a shared lock to an exclusive lock is also reported as a self-deadlock.

Deadlocked threads are marked with a \faArrowsSpin~icon in the notification area (section~\ref{mainwindow}) and on the timeline. Hovering over the icon lists all detected cycles, and clicking it opens the trace information window (section~\ref{traceinfo}), which lists each cycle with controls to focus it.

Remember that only instrumented locks are analyzed, so your application can still deadlock without notification if any locks are not instrumented.
