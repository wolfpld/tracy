...
When there are more visible data points than there are pixel columns available to draw them, the plot is instead displayed as a spectrogram. Each pixel column then holds a histogram of the values that fall within it, and the lightness of each cell indicates how many values are contained in it. Hovering the \faArrowPointer{}~mouse pointer over a column shows the number of values in the column, their range, and the number of values in the cell under the cursor.
